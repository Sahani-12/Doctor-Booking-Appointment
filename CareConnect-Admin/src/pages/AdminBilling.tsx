...
type Bill = {
  ...
  lineItems?: Array<{ description: string; quantity: number; unitPrice: number; amount: number; category?: string }>;
};
...
const tone = (status = "") => {
  ...
  if (v === "partially-paid") return "bg-sky-100 text-sky-700 dark:bg-sky-900/30 dark:text-sky-300";
  if (v === "pending") return "bg-amber-100 text-amber-700 dark:bg-amber-900/30 dark:text-amber-300";
  if (v === "cancelled") return "bg-rose-100 text-rose-700 dark:bg-rose-900/30 dark:text-rose-300";
...
export default function AdminBilling() {
  ...
  const loadAll = async () => {
    try {
      setLoading(true);
      const [billPayload, patientPayload, doctorPayload, deptPayload] = await Promise.all([
  ...
  const totalPending = bills.filter((b) => b.status !== "paid" && b.status !== "cancelled").reduce((sum, b) => sum + (b.balanceDue || 0), 0);
    ...
    return (
      <div className="flex min-h-[60vh] items-center justify-center">
        <div className="text-center">
          ...
          <p className="mt-3 text-sm text-slate-500 dark:text-slate-400">Loading billing data...</p>
        </div>
      </div>
    );
  }
    ...
    <div className="space-y-6 p-1">
      ...
      <div className="relative overflow-hidden rounded-3xl bg-gradient-to-br from-rose-600 via-pink-600 to-fuchsia-700 p-8 text-white shadow-xl">
        ...
        <div className="relative">
          <p className="text-xs font-bold uppercase tracking-[0.3em] text-rose-200/80">Hospital Admin</p>
          <h1 className="mt-3 text-4xl font-bold tracking-tight">Billing</h1>
          <p className="mt-2 max-w-xl text-sm text-rose-100/75">
            Issue bills, track collections, and settle outstanding balances across all patients.
          </p>
      ...
      <div className="grid gap-4 sm:grid-cols-3">
        {[
          { label: "Total Billed", value: money(totalBilled), color: "text-slate-900 dark:text-white", bg: "bg-white dark:bg-slate-900" },
          { label: "Collected", value: money(totalRevenue), color: "text-emerald-700 dark:text-emerald-300", bg: "bg-emerald-50 dark:bg-emerald-950/30" },
          { label: "Outstanding", value: money(totalPending), color: "text-rose-700 dark:text-rose-300", bg: "bg-rose-50 dark:bg-rose-950/30" },
        ].map((item) => (
          <div key={item.label} className={`rounded-3xl border border-slate-200 p-5 shadow-sm dark:border-slate-800 ${item.bg}`}>
      ...
      {showForm && (
        <div className="rounded-3xl border border-slate-200 bg-white p-6 shadow-sm dark:border-slate-800 dark:bg-slate-900">
          <div className="mb-5 flex items-center justify-between">
            <h2 className="text-xl font-semibold text-slate-900 dark:text-white">Issue new bill</h2>
          ...
          <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
            ...
            <div className="space-y-1.5 sm:col-span-2 lg:col-span-3">
              <label className="text-xs font-semibold uppercase tracking-wider text-slate-500 dark:text-slate-400">Notes</label>
              <textarea
                value={form.notes}
      ...
      {filtered.length === 0 ? (
        <div className="rounded-3xl border border-dashed border-slate-300 p-12 text-center dark:border-slate-700">
          ...
          <p className="mt-3 text-slate-500 dark:text-slate-400">No bills found.</p>
        </div>
      ) : (
        <div className="grid gap-4 lg:grid-cols-2">
          {filtered.map((bill) => (
            <div key={bill._id} className="rounded-3xl border border-slate-200 bg-white p-5 shadow-sm transition hover:shadow-md dark:border-slate-800 dark:bg-slate-900">
              <div className="flex items-start justify-between gap-3">
                <div>
                  <p className="font-mono text-xs font-bold text-slate-400 dark:text-slate-500">{bill.billNumber}</p>
                  <p className="mt-1 text-lg font-semibold text-slate-900 dark:text-white">{bill.patient?.fullname || "Patient"}</p>
                  <p className="text-sm text-slate-500 dark:text-slate-400">
                    {bill.department?.name || "Hospital services"}
                    {bill.doctor?.fullname ? ` · ${bill.doctor.fullname}` : ""}
                  </p>
                </div>
                <span className={`rounded-full px-3 py-1 text-xs font-semibold ${tone(bill.status)}`}>
                  {bill.status.replace("-", " ")}
                </span>
              </div>
                ...
                <div className="mt-4 space-y-1">
                  {(bill.lineItems || []).map((item, i) => (
                    <div key={i} className="flex items-center justify-between text-sm">
                      ...
                      <span className="font-medium text-slate-900 dark:text-white">{money(item.amount)}</span>
                    </div>
                  ))}
                </div>
              )}
              ...
              <div className="mt-4 grid grid-cols-3 gap-3">
                <div className="rounded-2xl bg-slate-50 px-3 py-2 dark:bg-slate-800">
                  ...
                  <p className="mt-1 font-semibold text-slate-900 dark:text-white">{money(bill.totalAmount)}</p>
                </div>
                <div className="rounded-2xl bg-emerald-50 px-3 py-2 dark:bg-emerald-950/20">
                  <p className="text-xs uppercase tracking-wide text-emerald-500">Paid</p>
                  <p className="mt-1 font-semibold text-emerald-700 dark:text-emerald-300">{money(bill.paidAmount)}</p>
                </div>
                <div className={`rounded-2xl px-3 py-2 ${bill.balanceDue > 0 ? "bg-rose-50 dark:bg-rose-950/20" : "bg-slate-50 dark:bg-slate-800"}`}>
                  <p className={`text-xs uppercase tracking-wide ${bill.balanceDue > 0 ? "text-rose-500" : "text-slate-400"}`}>Balance</p>
                  <p className={`mt-1 font-semibold ${bill.balanceDue > 0 ? "text-rose-700 dark:text-rose-300" : "text-slate-900 dark:text-white"}`}>{money(bill.balanceDue)}</p>
                </div>
              </div>

              <div className="mt-3 flex flex-wrap gap-3 text-xs text-slate-400 dark:text-slate-500">
                <span className="flex items-center gap-1"><CalendarDays className="h-3.5 w-3.5" /> Issued: {formatDate(bill.issuedAt)}</span>
              ...
              <div className="mt-4 flex flex-wrap gap-2 border-t border-slate-100 pt-4 dark:border-slate-800">
                {bill.balanceDue > 0 && bill.status !== "cancelled" && (
                  ...
                  >
                    {busyId === bill._id ? <Loader2 className="h-3.5 w-3.5 animate-spin" /> : <Check className="h-3.5 w-3.5" />}
                    Mark paid
                  </button>
                )}
