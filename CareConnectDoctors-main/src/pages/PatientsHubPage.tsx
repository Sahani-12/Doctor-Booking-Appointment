...
const tone = (status = "") => {
  ...
  if (["confirmed", "completed", "paid", "discharged"].includes(value))
    return "bg-emerald-500/15 text-emerald-300";
  if (["pending", "ordered", "admitted", "ready-for-discharge"].includes(value))
...
export default function PatientsHubPage() {
  ...
  const selectedAppointment = summary?.appointments.find(
    (item) => item._id === queryAppointmentId,
  );
  ...
  const loadSummary = async (patientId: string, isBackground = false) => {
    if (!patientId || !token) return;
    try {
      if (!isBackground) setSummaryLoading(true);
      setError("");
      ...
      if (!isBackground) setSummaryLoading(false);
    }
  };
    ...
    const load = async () => {
      try {
        const [patientPayload, departmentPayload] = await Promise.all([
          ...
          fetch(`${API_BASE}/hospital/departments`).then((response) =>
            response.json(),
          ),
        ]);
        ...
        const initialPatientId =
          queryPatientId &&
          roster.some((patient) => patient._id === queryPatientId)
            ? queryPatientId
            : roster[0]?._id || "";
        ...
        setError(
          err instanceof Error ? err.message : "Failed to load patient hub",
        );
      } finally {
        setLoading(false);
      }
  ...
  // Real-time polling for patient summary
  useEffect(() => {
    if (!selectedId || !token) return;

    const pollInterval = setInterval(() => {
      void loadSummary(selectedId, true);
  ...
  const openComposer = (
    ...
    item?: RecordItem | AdmissionItem | LabOrderItem,
  ) => {
    setMode(nextMode);
    ...
    if (!item) {
      setForm({
        ...emptyForm,
        appointmentId:
          nextMode === "record" || nextMode === "lab" ? queryAppointmentId : "",
        detail:
          nextMode === "record" && selectedAppointment?.notes
    ...
    setForm({
      ...emptyForm,
      tests: (order.tests || []).map((test) => test.name).join(", "),
    ...
    try {
      ...
      await request(`/hospital/admissions/${admission._id}`, {
        ...
        body: JSON.stringify({
          status,
          actualDischargeDate:
            status === "discharged" ? new Date().toISOString() : undefined,
        }),
      });
      await loadSummary(selectedId);
    } catch (err) {
      setError(
        err instanceof Error ? err.message : "Unable to update admission",
      );
    } finally {
      setBusyId("");
    }
  ...
  const updateLabStatus = async (order: LabOrderItem, status: string) => {
    try {
      ...
      setError(
        err instanceof Error ? err.message : "Unable to update lab order",
      );
    } finally {
      setBusyId("");
    }
    ...
    <div className="space-y-6">
      <section className="rounded-3xl bg-gradient-to-r from-slate-900 via-blue-950 to-cyan-950 p-8 text-white">
        ...
        <h1 className="mt-4 text-4xl font-semibold">
          Appointments now flow into records, admissions, and labs
        </h1>
      </section>
            ...
            {patients.map((patient) => (
              ...
              >
                ...
                <p className="mt-3 text-xs text-slate-500">
                  ...
                  {patient.activeAdmissions || 0} admissions
                </p>
              </button>
            ))}
          </div>
              ...
              <div className="rounded-3xl border border-slate-800 bg-slate-900/80 p-6">
                <div className="flex flex-wrap items-start justify-between gap-4">
                  <div>
                    ...
                    <p className="mt-1 text-sm text-slate-400">
                      {summary.patient.email || "Email unavailable"} |{" "}
                      {summary.patient.city || "City pending"}
                    </p>
                  </div>
                  <div className="grid gap-3 sm:grid-cols-5">
                    {[
                      {
                        label: "Appointments",
                        value: summary.stats.appointments,
                      },
                      ...
                      {
                        label: "Balance",
                        value: money(summary.stats.outstandingBalance),
                      },
                    ].map((item) => (
                      <div
                        key={item.label}
                ...
                {selectedAppointment && (
                  <div className="mt-6 rounded-2xl border border-cyan-500/20 bg-cyan-500/10 p-4 text-sm text-cyan-100">
                    Appointment context: {formatDate(selectedAppointment.date)}{" "}
                    at {selectedAppointment.slot}.{" "}
                    {selectedAppointment.notes ||
                      "Use this visit to create the next clinical note."}
                  </div>
                )}
                ...
                <div className="mt-6 flex flex-wrap gap-3">
                  <button
                    ...
                    className="rounded-2xl bg-cyan-500/15 px-4 py-2 text-sm font-medium text-cyan-300"
                  >
                    <NotebookPen className="mr-2 inline h-4 w-4" />
                    New record
                  </button>
                  <button
                    onClick={() => openComposer("admission")}
                    className="rounded-2xl bg-emerald-500/15 px-4 py-2 text-sm font-medium text-emerald-300"
                  >
                    <Stethoscope className="mr-2 inline h-4 w-4" />
                    New admission
                  </button>
                  <button
                    onClick={() => openComposer("lab")}
                    className="rounded-2xl bg-amber-500/15 px-4 py-2 text-sm font-medium text-amber-300"
                  >
                  ...
                  <div className="rounded-3xl border border-slate-800 bg-slate-900/80 p-6">
                    ...
                    <div className="mt-4 space-y-4">
                      {summary.appointments.map((appointment) => (
                        <div
                          key={appointment._id}
                          className={`rounded-2xl border p-4 ${appointment._id === queryAppointmentId ? "border-cyan-400 bg-cyan-500/10" : "border-slate-800 bg-slate-950/60"}`}
                        >
                          <div className="flex flex-wrap items-start justify-between gap-3">
                            ...
                            <span
                              className={`rounded-full px-3 py-1 text-xs font-semibold ${tone(appointment.status)}`}
                            >
                              {appointment.status}
                            </span>
                          </div>
                      ...
                      {summary.records.map((record) => (
                        ...
                        >
                          <div className="flex flex-wrap items-start justify-between gap-3">
                            ...
                            <button
                              onClick={() => openComposer("record", record)}
                              className="rounded-2xl bg-slate-800 px-3 py-2 text-xs font-medium text-slate-200"
                            >
                              Edit
                            </button>
                          </div>
                      ...
                      {summary.admissions.map((admission) => (
                        ...
                        >
                          <div className="flex flex-wrap items-start justify-between gap-3">
                            ...
                            <span
                              className={`rounded-full px-3 py-1 text-xs font-semibold ${tone(admission.status)}`}
                            >
                              {admission.status}
                            </span>
                          </div>
                          ...
                          <div className="mt-4 flex flex-wrap gap-2">
                            <button
                              onClick={() =>
                                openComposer("admission", admission)
                              }
                              className="rounded-2xl bg-slate-800 px-3 py-2 text-xs font-medium text-slate-200"
                            >
                              Edit
                            ...
                            {admission.status !== "ready-for-discharge" &&
                              admission.status !== "discharged" && (
                                <button
                                  onClick={() =>
                                    void updateAdmissionStatus(
                                      admission,
                                      "ready-for-discharge",
                                    )
                                  }
                                  disabled={busyId === admission._id}
                                  className="rounded-2xl bg-amber-500/15 px-3 py-2 text-xs font-medium text-amber-300 disabled:opacity-50"
                                >
                                  Ready
                                </button>
                              )}
                            {admission.status !== "discharged" && (
                              <button
                                ...
                                disabled={busyId === admission._id}
                                className="rounded-2xl bg-emerald-500/15 px-3 py-2 text-xs font-medium text-emerald-300 disabled:opacity-50"
                              >
                                Discharge
                              </button>
                            )}
                    ...
                    <div className="mt-4 space-y-4">
                      {summary.labOrders.map((order) => (
                        ...
                        >
                          ...
                          <div className="mt-4 flex flex-wrap gap-2">
                            <button
                              onClick={() => openComposer("lab", order)}
                              className="rounded-2xl bg-slate-800 px-3 py-2 text-xs font-medium text-slate-200"
                            >
                              Edit
                            </button>
                            {order.status !== "processing" &&
                              order.status !== "completed" && (
                                <button
                                  onClick={() =>
                                    void updateLabStatus(order, "processing")
                                  }
                                  disabled={busyId === order._id}
                                  className="rounded-2xl bg-sky-500/15 px-3 py-2 text-xs font-medium text-sky-300 disabled:opacity-50"
                                >
                                  Processing
                                </button>
                              )}
                            {order.status !== "completed" && (
                              <button
                                onClick={() =>
                                  void updateLabStatus(order, "completed")
                                }
                                disabled={busyId === order._id}
                                className="rounded-2xl bg-emerald-500/15 px-3 py-2 text-xs font-medium text-emerald-300 disabled:opacity-50"
                              >
